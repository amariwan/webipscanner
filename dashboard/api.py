...
def genPDF(request):
    ...
    if 'xmlfile' in request.session:
        ...
        if os.path.exists('dashboard/static/' + pdffile + '.pdf'):
            os.remove('dashboard/static/' + pdffile + '.pdf')

        os.popen(
            '/usr/local/bin/Webipscanner --cookie sessionid ' + request.session._session_key + ' --enable-javascript --javascript-delay 6000 http://127.0.0.1:8888/view/pdf/ dashboard/static/' + pdffile + '.pdf')
...
def getCVE(request):
    ...
    if request.method == "POST":
        scanfilemd5 = hashlib.md5(str(request.session['xmlfile']).encode('utf-8')).hexdigest()
        cveproc = os.popen('python3 dashboard/nmap/cve.py ' + request.session['xmlfile'])
...
def apiv1_scan(request):
    ...
    r['Webipscanner_version'] = gitcmd.read().strip()
