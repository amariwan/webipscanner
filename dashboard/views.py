...
def login(request):
	...
	return render(request, 'dashboard/nmap_auth.html', r)
...
def setscanfile(request, xmlfile):
	...
	return render(request, 'dashboard/nmap_hostdetails.html', { 'js': '<script> location.href="/"; </script>' })


def port(request, port):
	return render(request, 'dashboard/index.html', { 'out': '', 'table': '', 'scaninfo': '', 'scandetails': '', 'trhost': '' })
...
def details(request, address):
	...
	for ik in o['host']:
		...
		if str(saddress) == address:
			...
			if re.search('^Webipscannersched\_[0-9\.]+', request.session['xmlfile']):
				m = re.search('^Webipscannersched\_[0-9\.]+\_(.+)', request.session['xmlfile'])
	...
	return render(request, 'dashboard/nmap_portdetails.html', r)
...
def index(request, filterservice="", filterportid=""):
	...
	gitcmd = os.popen('cd dashboard && git rev-parse --abbrev-ref HEAD')
	r['Webipscannerver'] = 'Webipscanner '+gitcmd.read()+'<br>This project is currently a beta, please <b>DO NOT</b> expose Webipscanner to internet.<br>This version is <b>1.0.0</b> production ready.'
	...
	if 'xmlfile' in request.session:
		...
		for i in xmlfiles:
			...
			filename = i
			if re.search('^Webipscannersched\_[0-9\.]+',i):
				m = re.search('^Webipscannersched\_([0-9\.]+)\_(.+)',i)
		...
		return render(request, 'dashboard/nmap_xmlfiles.html', r)
	...
	return render(request, 'dashboard/nmap_hostdetails.html', r)
...
def scan_diff(request, f1, f2):
	...
	return render(request, 'dashboard/nmap_ndiff.html', r)
...
def about(request):
	...
	return render(request, 'dashboard/nmap_about.html', r)
